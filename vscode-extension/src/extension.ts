...
export function activate(context: vscode.ExtensionContext) {
    ...
    const compileCommand = vscode.commands.registerCommand('ezui.compile', async (uri: vscode.Uri) => {
        if (!uri) {
    ...
    const previewCommand = vscode.commands.registerCommand('ezui.preview', async (uri: vscode.Uri) => {
        if (!uri) {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                vscode.window.showErrorMessage('No EZUI file selected');
    ...
    const hoverProvider = vscode.languages.registerHoverProvider('ezui', {
        provideHover(document, position) {
            ...
            const ezuiKeywords: { [key: string]: string } = {
...
async function compileEZUI(content: string): Promise<string> {
...
async function generatePreview(content: string, filePath: string): Promise<string> {
